...
| [`ui/standards.md`](ui/standards.md) | UI standards and rules (R0-R12): Material-3-only, token foundation, shared components (`Preference.kt`, `Dialog.kt`, `InfoCard`, `AppBarSearchField`), screen skeleton, strings policy, dialogs, D-pad/accessibility (focus, scrub, labels), and the test harnesses. |
...
| [`../tests/dpad/README.md`](../tests/dpad/README.md) | adb-driven D-pad / no-touchscreen test harness: hard-data oracles (play/pause, seek, reorder, BACK) + per-screen focus-coverage sweep, onboarding through every screen. |
| [`../tests/unification/check.mjs`](../tests/unification/check.mjs) | Static UI-unification/reusability checks (audit clean, no duplicated search field, every interactive IconButton labelled, no raw dialogs, shared components present). |
